    ...
    if (params['*']) {
      ...
      pathNames.forEach((path, index) => {
        if (currentNode.id === path && index === pathNames.length - 1) {
          ...
          const foundNode = currentNode.children?.find((el) => el.id === path)
    ...
    <FolderLayout>
      ...
      {currentFolderData.files.map((el) => (
        <FolderButton
          key={el.id}
          text={el.fileName}
          icon={<DocumentIcon className="mx-auto h-12 w-12 text-inherit" />}
          isFile={true}
        />
      ))}
      <FolderButton
        icon={<FolderPlusIcon className="mx-auto h-12 w-12 text-gray-500" />}
        dashed={true}
        text={'Создать папку'}
      />
      <FolderButton
        icon={<DocumentPlusIcon className="mx-auto h-12 w-12 text-gray-500" />}
        dashed={true}
